...
const GenerateCourierReport = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/reports/generate-branch-admin", {
        headers: { Authorization: `Bearer ${token}` },
        params,
      });
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/reports/export-branch-admin", {
        headers: { Authorization: `Bearer ${token}` },
        params,
        responseType: "blob",
      });
